...
export class ApiService {
  ...
  notificationRecieved$ = this.notificationSource.asObservable();
	...
	constructor(public http: Http, storage: Storage, public platform: Platform, public config: Config) {
    ...
    Object.keys(params).map(function (value) {
      search.set(value, params[value]);
		...
		return this.http.get(this.serverUrl + url, {headers: this.headers, search: search})
      .map(res => res.json())
      .catch((error) => {
  ...
  public put (url: string, data: any): Observable<any> {
    ...
    return this.http.delete(this.serverUrl + url, {headers: this.headers})
      .map(res => res.json())
      .catch((error) => {
        return this.handleError(error);
      });
  ...
  public refreshDeviceToken(token: string, location_id?: number): Promise<any> {
	  if(token){
    ...
    if(this.isAuth()){
      ...
      return this.put('users/refresh-token', {platform: platform, device_token: this.deviceToken, location_id: location_id}).toPromise();
  ...
  public removeDeviceToken(): Promise<any> {
      ...
      return this.put('users/remove-token', {platform: platform, device_token: this.deviceToken}).toPromise();
    }else{
  ...
  public makeFileRequest (url: string, params: any, file?: File): Observable<any> {
    return Observable.create(observer => {
      ...
      xhr.setRequestHeader('Authorization', 'Bearer ' + this.user.api_token);
      xhr.send(formData);
    });
  }
