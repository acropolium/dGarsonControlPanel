...
export class OrdersPage {
  ...
  constructor(public navCtrl: NavController, private apiService: ApiService, private popoverCtrl: PopoverController, public translate: TranslateService, public params: NavParams) {
  ...
  setState(order, state){
    order.loading = true;
    this.apiService.put('orders/'+order.id, {state: state}).subscribe(
      data => {
  ...
  presentPopover(ev) {
    this.translate.get(['created_at', 'take_away_time']).subscribe((translation: string) => {
      let popover = this.popoverCtrl.create(SortPopover, {
        sorting: [{name: translation['created_at'], value: "created_at"}, {name: translation["take_away_time"], value: "take_away_time"}],
        selected: this.order
      });
  ...
  trackByOrders(index: number, order: any) { return order.id; }

  getOrders(after?: boolean, nextPage?: boolean): Promise<any>{
    let params = {company_id: this.companyFilter, location_id: this.locationFilter, order: this.order, state:[]};
    let states = {'new': ['pending', 'recieved'], 'process':['inprogress', 'ready']};
