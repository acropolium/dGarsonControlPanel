...
export class CompaniesPage {
    ...
    constructor(
        ...
        this.getCompaniesData({
            ...
            group_statistic_by: this.group_statistic_by,
        });
        ...
        this.translate
            .get([this.chartOptions.title])
            .subscribe(
                translation =>
                    (this.chartOptions.title =
                        translation[this.chartOptions.title])
            );
    }
    ...
    getCompaniesData(range = {}) {
        ...
        if (this.items) {
            ...
            params = Object.assign({}, params, {
                locations_allow: this.showLocations,
            });
        }

        let showLocations = this.showLocations;
        this.apiService.get('users/companies', params).subscribe(data => {
            data.forEach(item => {
                item.locations.forEach(location => {
                    location.isShowStatistic =
                        showLocations.indexOf(location.id) !== -1 ||
    ...
    presentPopoverLocation(company, location, event) {
        ...
        let popover = this.popoverCtrl.create(PopoverLocationsPage, {
            company: company,
            location: location,
            ev: event,
        });
    ...
    removeCompany(item) {
        this.apiService.delete('companies/' + item.id).subscribe(data => {
    ...
    presentCompanyActionSheet(item) {
        this.showForm = false;
        this.translate
            .get(['Choose action', 'Locations', 'Cancel', 'Edit', 'Delete'])
    ...
    presentEditModal(item) {
        this.translate
            ...
            .subscribe((translation: string) => {
                let companyEditModal = this.modalCtrl.create(CompanyEdit, {
                    item: item,
    ...
    presentLocationEditModal(company, item) {
        ...
        let menuEditModal = this.modalCtrl.create(LocationEdit, {
            item: item,
            currentCompany: company,
            apiService: this.apiService /*, translation: translation*/,
        });
    ...
    removeLocation(company, location) {
        this.apiService.delete('locations/' + location.id).subscribe(data => {
