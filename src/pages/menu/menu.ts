...
export class MenuPage {
    ...
    constructor(
        ...
        if (this.currentCompany) {
            this.menuService
                .get('companies/' + this.currentCompany.id + '/menu', {
                    per_page: 500,
                ...
                .subscribe(data => {
                    ...
                    data.data.map(menu => {
                        if (menu.location_id) {
                            this.menuService
                                .get('location/' + menu.location_id)
                                .subscribe(location => {
                                    menu.location = location[0]
                                        ? location[0]
                                        : null;
                                });
                            return menu;
    ...
    removeMenu(item) {
        this.menuService.delete('menu/' + item.id).subscribe(data => {
    ...
    presentPopover(item, event) {
        this.translate
            .get(['Edit', 'Delete', 'delete_confirm'])
            .subscribe((translation: string) => {
                let popover = this.popoverCtrl.create(PopoverMenuItem, {
                    item: item,
                    ev: event,
    ...
    presentEditModal(item, $event) {
        this.translate
            ...
            .subscribe((translation: string) => {
                let menuEditModal = this.modalCtrl.create(MenuEdit, {
                    item: item,
                ...
                menuEditModal.onDidDismiss(data => {
                    if (data) {
                        if (
                            data.location_id &&
                            data.location_id != this.currentLocation.id
                        ) {
                            let index = this.items.indexOf(item, 0);
                            if (index > -1) {
                                this.items.splice(index, 1);
